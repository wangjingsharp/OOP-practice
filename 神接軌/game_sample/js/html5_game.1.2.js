...
var Framework = (function (Framework) {
    ...
    return Framework;
 })(Framework || {});;var Framework = (function (Framework) {
	'use strict'
    ...
    Framework.Sprite = Framework.exClass(Framework.GameObject , {
        ...
        */
	    ...
	    setRotation: function(rotation){
	    	if (this.absoluteRotation == rotation) return;
	        this.absoluteRotation = rotation;
	        this.drawTmp();
	    },
	    setScale: function(scale){
	    	if (this.absoluteScale == scale) return;
	    ...
	    setX: function(x){
	    	if (this.absolutePosition.x == x) return;
	        this.absolutePosition.x = x;
	    },
	    setY: function(y){
	    	if (this.absolutePosition.y == y) return;
	    ...
	    drawTmp: function(){
	        ...
	        this.tmpCanvas.width = diagonalLength;
	    	this.tmpCanvas.height = diagonalLength;
	    	
	    	//取得繪圖內容
	    	this.tmpContext = this.tmpCanvas.getContext("2d");
	    	
			//旋轉
			if (this.absoluteRotation % 360){
				var tranlateX = this.tmpCanvas.width / 2, tranlateY = this.tmpCanvas.height / 2;
				this.tmpContext.translate(tranlateX , tranlateY);
				this.tmpContext.rotate(this.absoluteRotation / 180 * Math.PI);
				this.tmpContext.translate(-tranlateX , -tranlateY);
			}
			
			//縮放
			if (this.absoluteScale != 1){
				this.tmpContext.scale(this.absoluteScale, this.absoluteScale);
			}
			
			// 畫圖                
			this.tmpContext.drawImage(this.texture, (this.tmpCanvas.width - realWidth) / 2 / this.absoluteScale, (this.tmpCanvas.height - realHeight) / 2 / this.absoluteScale);
			
			//顯示邊框
			if (this.isDrawBoundry) {
				this.tmpContext.rect((this.tmpCanvas.width - realWidth) / 2 / this.absoluteScale, (this.tmpCanvas.height - realHeight) / 2 / this.absoluteScale, this.texture.width, this.texture.height);                    
			} 
	    ...
	    countAbsoluteProperty: function() {
		    ...
		    var rad = (parentRotation / 180) * Math.PI;
		    this.setRotation(this.rotation + parentRotation);
		    this.setScale(this.scale * parentScale);
		    this.setPosition(
		    	Math.floor((this.relativePosition.x * Math.cos(rad) - this.relativePosition.y * Math.sin(rad))) * parentScale + parentPositionX,
		    	Math.floor((this.relativePosition.x * Math.sin(rad) + this.relativePosition.y * Math.cos(rad))) * parentScale + parentPositionY
		    );
		},
	    ...
	    draw: function(context){
	        ...
	        if(this.type === 'image' || this.type === 'canvas'){
	            ...
	            context.drawImage(this.tmpCanvas, this.absolutePosition.x - this.tmpCanvas.width / 2, this.absolutePosition.y - this.tmpCanvas.height / 2);
	        }
	    },    
	    toString:function(){
...
var Framework = (function (Framework) {
    ...
    */ 
	Framework.Game = (function () {
		...
		that._canvas.width = 1200;
		that._canvas.height = 800;
...
/**/
		...
		*/
		...
		that.resizeEvent = function() {
			...
			var base = 0,
				baseWidth = window.innerWidth / 8,
				baseHeight = window.innerHeight / 8,
				scaledWidth = 0,
				scaledHeight = 0;
			...
			scaledWidth = Math.round(base * 8);
			scaledHeight = Math.round(base * 8);
        ...
        */
		var play = function(audioArgs) {
			var sourceTagStr = 'source',
            	...
            	audio = {};
			oggSource.currentTime = 0;
			mp3Source.currentTime = 0;
...
delete Box2D.postDefs;;var Framework;
Framework = (function (Framework) {	
	'use strict'
